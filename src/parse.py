...
Enhanced argument parser with model selection and separate eigenvalue counts
...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='enhanced', 
                       ...
                       help='Model type: basic (model.py) or enhanced (model_enhanced.py)')
...
def validate_args(args):
    """Enhanced argument validation with model selection and eigenvalue parameter checking"""
    ...
    if args.u_n_eigen > 500 or args.i_n_eigen > 500:
        print(f"⚠️  Warning: Very high eigenvalue counts may cause memory issues")
        
    if args.u_n_eigen > 0 and args.u_n_eigen < 16:
        print(f"⚠️  Warning: u_n_eigen={args.u_n_eigen} is very low, may hurt performance")
        
    if args.i_n_eigen > 0 and args.i_n_eigen < 16:
        print(f"⚠️  Warning: i_n_eigen={args.i_n_eigen} is very low, may hurt performance")
    ...
    if args.use_laplacian:
        if not (0.001 <= args.similarity_threshold <= 0.5):
            print(f"⚠️  Warning: similarity_threshold {args.similarity_threshold} may be extreme for Laplacian mode")
    
    if args.epochs < 10:
        print(f"⚠️  Warning: Very few epochs ({args.epochs}), consider increasing")
    
    # Model-specific warnings
    if args.model_type == 'basic' and args.filter_order > 6:
        print(f"💡 Basic model works well with filter_order <= 6")
...
if __name__ == "__main__":
    ...
    print()
    print("# Auto-adaptive enhanced:")
    print("python main.py --model_type enhanced --dataset gowalla --filter_design multiscale")
    print()
    print("# Legacy mode:")
    print("python main.py --model_type basic --dataset ml-100k --n_eigen 50")