...
def Test(dataset, Recmodel, epoch, multicore=0):
    ...
    if(world.simple_model == 'lgn-ide'):
        ...
        lm.train()
    elif(world.simple_model == 'pu-sp-cf'):
        # Use the main Recmodel which should be PureUniversalSpectralCF
        lm = Recmodel
        lm.eval()    
    ...
    with torch.no_grad():
        ...
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            ...
            if(world.simple_model == 'pu-sp-cf'):
                # Use PureUniversalSpectralCF's getUsersRating method (only needs batch_users_gpu)
                rating = lm.getUsersRating(batch_users_gpu)
                rating = torch.from_numpy(rating) if isinstance(rating, np.ndarray) else rating
            elif(world.simple_model != 'none'):
                # Other simple models need batch_users (CPU) and world.dataset
