...
def MSE_train_simple(dataset, model, loss_class, epoch):
    """
    Direct MSE training - no negative sampling needed!
    ...
    # Training parameters
    n_batch = 10  # Even fewer batches needed
...
def train_universal_spectral_simple(dataset, model, config, total_epochs=15):
    """
    Complete simplified training procedure for Universal Spectral CF
    Direct MSE on rating matrix - much faster convergence!
    """
    print("="*70)
    print("STARTING DIRECT MSE TRAINING (NO NEGATIVE SAMPLING)")
    print("="*70)
    ...
    for epoch in tqdm(range(total_epochs), desc="Training Progress"):
        ...
        # Print progress every 3 epochs (since we have fewer epochs)
        if epoch % 3 == 0:
...
def simple_train_and_evaluate(dataset, model, config):
    ...
    trained_model = train_universal_spectral_simple(
        ...
        total_epochs=world.config.get('epochs', 100)
    )
