...
class UniversalSpectralCF(nn.Module):
    ...
    def forward(self, users, target_ratings=None):
        """Memory-efficient forward pass with positive and negative similarities"""
        ...
        if target_ratings is not None:
            ...
            filters = [self.user_pos_filter, self.user_neg_filter, self.item_pos_filter, self.item_neg_filter]
            reg_loss = sum(f.coeffs.norm(2).pow(2) for f in filters if f is not None)
